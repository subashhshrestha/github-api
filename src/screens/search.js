...
export default class Search extends Component {
  ...
  onChange(e) {
    ...
    fetch(`https://api.github.com/search/users?q=${e.target.username.value}`)
      .then(res => res.json())
      .then(data => {
  ...
  getUser(username){
    ...
    fetch(`https://api.github.com/users/${username}`).then(res=>res.json()).then(data1=>{
    console.log(data1)  
    this.setState({
      userInfo:data1,
      show:true
    ...
    fetch(`https://api.github.com/users/${username}/repos`).then(res=>res.json()).then(datas=>{
      console.log("Repo",datas);
              this.setState({
                repo:datas
              })
            })
  ...
  render() {
    return (
      ...
      <div className="searchPart">
        <div className="forms">
                <form className="formss" onSubmit={this.onChange}>
                <input id="searchbar" type='text' name="username" placeholder=""/><br/>
                <button id="searchbtn">Search</button>
                {/* <Link to = '/search'>Search</Link> */}
            </form>
            </div>
        <div className="searchResult">
            {this.state.item.map(user=><List user={user} click={this.getUser}/>)}
        </div>
        </div>
        {this.state.show?<div className="searchProfile">
        <div className="imageParent">
         ...
         <div className="searchdescription">
            <div className="searchusername"><h1>{this.state.userInfo.name}</h1></div>
          <div className="bar">
          <div><span className="descKey">Repositories </span><span className="descValue">{this.state.userInfo.public_repos}</span></div>
          <div><span className="descKey">Stars </span><span className="descValue">{this.state.userInfo.public_gists}</span></div>
          <div><span className="descKey">Followers </span><span className="descValue">{this.state.userInfo.followers}</span></div>
          <div><span className="descKey">Following </span><span className="descValue">{this.state.userInfo.following}</span></div>
          </div>
          <div className="repos">
            {this.state.repo.map(repo=><Repo repo={repo} color={this.state.color}/>)}
          </div>
         </div>
        </div>:''}
...
const List=(props)=>{
    return(
      <div className="userlists" onClick={()=>props.click(props.user.login)}>
...
const Repo=(props)=>{
  return(
    <div className="repolist">
      <span style={{fontWeight:'bold',color:'#0366D6',fontSize:'15px',overflowWrap:'break-word'}}>{props.repo.name}</span>
      <span style={{color:'#586069',fontSize:'12px',marginTop:'10px'}}>{props.repo.description}</span>
      {props.repo.language?
      <div style={{color:'#586069',fontSize:'13px',marginTop:'20px',display:'flex',alignItems:'center'}}><div style={{borderRadius:'50%',backgroundColor:`${props['color'][`${props.repo.language}`]['color']}`,width:'15px',height:'15px',marginRight:'5px'}}></div><div>{props.repo.language}</div></div>
      :''}
    </div>
  )
}
